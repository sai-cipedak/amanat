...
const els = {
  ...
  volunteerApplicationCount:$("volunteer-application-count"),
  volunteerApplicationNote:$("volunteer-application-note"),
  volunteerMatchedCount:$("volunteer-matched-count"),
...
async function loadVolunteerAccountData(){
  if(!state.session?.user){
  ...
  const [summary,applications,updates]=await Promise.all([
    db.rpc("get_my_volunteer_summary"),
    db.rpc("get_my_volunteer_applications"),
    db.rpc("get_my_contributor_progress_updates")
  ]);

  for(const result of [summary,applications,updates]){
  ...
  state.myApplications=applications.data||[];
  state.contributorProgressUpdates=updates.data||[];
}
...
function renderVolunteerAccount(){
  const loggedIn=Boolean(state.session?.user);
  els.volunteerAccount.hidden=!loggedIn;
  if(!loggedIn)return;

  const summary=state.volunteerSummary||{};
  els.volunteerApplicationCount.textContent=
    Number(summary.my_applications||0);
  els.volunteerApplicationNote.textContent=
    `${Number(summary.pending_applications||0)} pending`;
  els.volunteerMatchedCount.textContent=
    Number(summary.matched_opportunities||0);
  els.volunteerOpenCount.textContent=
    Number(summary.open_opportunities||0);
  els.volunteerActiveCount.textContent=
    Number(summary.active_contributions||0);

  renderVolunteerApplications();
  renderVolunteerContributions();
}

function renderVolunteerApplications(){
  if(!state.myApplications.length){
    els.volunteerApplicationList.innerHTML=`
      <div class="empty-card">
        Belum ada application. Cari opportunity di halaman yang sama,
        lalu klik Join.
      </div>`;
    return;
  }
  ...
  els.volunteerApplicationList.innerHTML=
    state.myApplications.map(app=>`
      <article class="volunteer-account-row">
        <div>
          <div class="card-badges">
            <span class="badge">${esc(statusLabel(app.application_status))}</span>
          </div>
          <h3>${esc(app.opportunity_title||app.metric_name)}</h3>
          <div class="volunteer-account-meta">
            ${esc(app.kpi_id)} · ${esc(app.kpi_title)}<br>
            ${esc(app.metric_id)} · Submitted ${esc(formatDateId(app.submitted_at))}
          </div>
        </div>
        <div class="volunteer-row-actions">
          <span class="badge">
            ${esc(MODE_LABELS[app.applied_contribution_mode]||app.applied_contribution_mode||"—")}
          </span>
          ${
            app.contributor_status
              ? `<span class="badge active">
                   Assignment: ${esc(statusLabel(app.contributor_status))}
                 </span>`
              : ""
          }
        </div>
...
function activeContributions(){
  return state.myApplications.filter(
...
function renderVolunteerContributions(){
  ...
  els.volunteerContributionList.innerHTML=
    contributions.map(app=>{
      const pending=contributorUpdatesForMetric(app.metric_id)
        .filter(update=>update.update_status==="pending").length;
        ...
        <article class="volunteer-account-row">
          <div>
            <div class="card-badges">
              ...
              ${
                pending
                  ? `<span class="badge">${pending} pending owner review</span>`
                  : ""
              }
            </div>
          ...
          <div class="volunteer-row-actions">
            <button class="primary"
                    type="button"
                    data-contributor-progress="${esc(app.metric_id)}">
              Submit Progress
            </button>
          </div>
        </article>
  ...
  els.volunteerContributionList
    .querySelectorAll("[data-contributor-progress]")
    .forEach(button=>{
      button.addEventListener("click",()=>{
        openContributorProgress(button.dataset.contributorProgress);
      });
    });
}

function selectedContribution(){
  return activeContributions().find(
    app=>app.metric_id===state.selectedContributionMetricId
  )||null;
}

function setContributorMessage(text,type=""){
  message(els.contributorProgressMessage,text,type);
}

function renderContributorHistory(){
  const metricId=state.selectedContributionMetricId;
  const rows=contributorUpdatesForMetric(metricId);
  els.contributorProgressHistoryCount.textContent=rows.length;

  if(!rows.length){
    els.contributorProgressHistory.innerHTML=
      `<div class="empty-card">Belum ada contributor progress submission.</div>`;
    return;
  }

  els.contributorProgressHistory.innerHTML=rows.map(row=>`
    <article class="volunteer-account-row">
      <div>
        <div class="card-badges">
          <span class="badge">${esc(statusLabel(row.update_status))}</span>
        </div>
        <h3>As of ${esc(row.as_of_date||"—")}</h3>
        <div class="volunteer-account-meta">
          Actual: ${esc(row.actual??"—")}
          · Progress:
          ${row.progress_pct==null?"—":`${esc(row.progress_pct)}%`}
          ${row.update_note?`<br>${esc(row.update_note)}`:""}
          ${row.owner_review_note?`<br>Owner: ${esc(row.owner_review_note)}`:""}
        </div>
      </div>
    </article>
  `).join("");
...
function openContributorProgress(metricId){
  const app=activeContributions().find(
  ...
  els.contributorProgressContext.innerHTML=`
    <strong>${esc(app.kpi_id)} · ${esc(app.kpi_title)}</strong><br>
    ${esc(app.metric_id)} · ${esc(app.metric_name)}<br>
    Assignment:
    <strong>${esc(MODE_LABELS[app.assigned_contribution_mode]||
      app.assigned_contribution_mode||"—")}</strong>
  `;

  els.contributorProgressDate.value=
  ...
  renderContributorHistory();
  els.contributorProgressModal.hidden=false;
}

async function submitContributorProgress(){
  const actualRaw=els.contributorProgressActual.value.trim();
  const progressRaw=els.contributorProgressPct.value.trim();

  if(!actualRaw&&!progressRaw){
    throw new Error("Isi Actual atau Progress Estimate.");
  }

  const {error}=await db.rpc(
    "submit_contributor_progress_update",
    {
      p_metric_id:state.selectedContributionMetricId,
      p_as_of_date:els.contributorProgressDate.value,
      p_actual:actualRaw===""?null:Number(actualRaw),
      p_progress_pct:progressRaw===""?null:Number(progressRaw),
  ...
  await loadVolunteerAccountData();
  renderVolunteerAccount();
  renderContributorHistory();
...
function renderAuth(){
  if(!state.session?.user){
    ...
    els.volunteerAccount.hidden=true;
    return;
  }
  ...
  els.profileBtn.textContent=
    state.profile?"Update Profile":"Lengkapi Profile";
...
async function saveProfile(){
  ...
  state.profile=p;
  state.profileSkillIds=ids;
  state.profileModes=modes;
  await loadVolunteerAccountData();
  renderAuth();
}
...
async function submitApplication(){
  ...
  await Promise.all([loadUserData(),loadVolunteerAccountData()]);
...
els.matchedSummaryCard.onclick=async()=>{
  ...
  els.capacityFilters.scrollIntoView({
    behavior:"smooth",
    block:"start"
  });
};

els.contributorProgressClose.onclick=()=>{
...
els.contributorProgressForm.onsubmit=async event=>{
  event.preventDefault();
  setContributorMessage("Submitting…");
  try{
    ...
    setContributorMessage(err.message,"error");
  }
};
...
db.auth.onAuthStateChange(async(_event,session)=>{
  ...
  renderAuth();
  renderList();
  if(session)await handlePending();
});
